...
class DeliveryMethod(models.Model):
    title = models.CharField(max_length=78, unique=True, validators=[validators.MinLengthValidator(3)],
                             blank=False, null=False)
    category = models.ForeignKey(DeliveryCategory, on_delete=models.PROTECT,
                                 blank=False, null=False,
    ...
    cities = models.JSONField(default=dict,
                              blank=True, null=True)
    need_tracking_code = models.BooleanField(default=False,
                                             blank=False, null=False, verbose_name='Need Tracking Code')
    is_active = models.BooleanField(default=True,
                                    blank=False, null=False, verbose_name='Is Active')
    main_price = models.PositiveIntegerField(default=0,
                                             blank=False, null=False, verbose_name='Main Price')
    is_public = models.BooleanField(default=True,
                                    blank=False, null=False, verbose_name='Is Public')
    roles = models.ManyToManyField(
        Role,
        through='DeliveryMethodForRole',
...
class DeliveryMethodForRole(models.Model):
...
class DeliveryStatus(models.TextChoices):
    PENDING = 'PEN', 'در انتظار'
    REQUESTED = 'REQ', 'درخواست'
    SUBMITTED = 'SUB', 'ثبت شده'
    ENDED = 'END', 'تحویل داده شده'


class DeliveryReceipt(models.Model):
    delivery_method = models.ForeignKey(DeliveryMethod, on_delete=models.PROTECT,
                                        blank=False, null=False, verbose_name='Delivery Method',
    ...
    status = models.CharField(max_length=3, validators=[validators.MinLengthValidator(3)],
                              ...
                              blank=False, null=False)
    delivery_employee = models.ForeignKey(Employee, on_delete=models.SET_NULL,
                                          blank=True, null=True, verbose_name='Delivery Employee',
                                          related_name='submit_delivery_receipt_list')
    ...
    def __str__(self):
        return f"Delivery Receipt #{self.pk} /For: {self.receiver}"
